...
| ARCH-EXT-03 | Recurring rule sub-document on expenses | Architecture | `expenses` collections, optional field `recurringRule` | Always `null`. Written on every expense document at creation time. | When non-null, contains `{ frequency, interval, endDate, nextOccurrence }`. A scheduled Cloud Function creates new expense documents on recurrence and advances `nextOccurrence`. | Recurring expenses |
...
| ARCH-EXT-05 | Notification channel expansion | Architecture | Cloud Functions notification module (`functions/src/triggers/`) | FCM push notifications only via a `NotificationChannel` strategy pattern with a single `FcmChannel` implementation. | Additional channel implementations: `SmsChannel`, `EmailChannel`, `InAppInboxChannel`. Channel selector reads user preferences from `notificationPrefs` sub-document. | Notification channels |
...
| ID | Phase | Seam summary |
|---|---|---|
| ARCH-EXT-05 | Architecture | Strategy/dispatcher pattern in the Cloud Functions notification module with pluggable channel implementations |

**Activation sequence:** Single seam. The v1.0 notification module already uses a strategy pattern with `FcmChannel` as the sole implementation. Adding a new channel (SMS, email, in-app inbox) requires implementing the `NotificationChannel` interface and registering it in the dispatcher. No schema or IA changes are needed; user preference controls in the Profile screen are the only client-side addition.
