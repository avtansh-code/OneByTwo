...
persistence (`settings.persistenceEnabled = true`) as the sole local data
store. No secondary persistence layer (Hive, Isar, SQLite) is introduced.
This decision is recorded in ADR-0014 and is motivated by three factors:
...
The Firestore cache size is configured to **100 MB** (the SDK default). This
is sufficient for the v1.0 data model, which consists of lightweight JSON
documents (expenses, settlements, friendships, groups). Should telemetry
indicate cache eviction pressure on low-storage devices, the limit can be
raised or set to `CACHE_SIZE_UNLIMITED` via a Remote Config flag in a future
release.
...
- Group documents (including their `simplifiedBalances` maps).
- Expense and settlement documents for groups and friendships the user has
  opened.
...
The application displays a persistent **"Offline" banner** at the top of the
screen (below the app bar) whenever the device loses connectivity. The banner
is driven by a connectivity-state provider that listens to the platform's
network-reachability events. The banner is dismissed automatically when
connectivity is restored.

The banner uses the `Coral` danger colour token from the design system
(SRS section 6.2) at reduced opacity to avoid alarm, paired with a brief
...
with `SnapshotMetadata.isFromCache == true`. The UI does not distinguish cached
snapshots from server snapshots at the data layer; the offline banner is the
sole user-facing indicator.
...
Because `persistenceEnabled` is set to `true`, pending writes are persisted to
disk. If the user kills the app, restarts the device, and later opens the app
with connectivity, all queued writes are transmitted to Firestore in the order
they were issued.
...
local cache rolls back and the listener emits the corrected state. The UI
should display a snackbar informing the user that the write could not be saved.
...
user, the application notifies the affected user. The notification mechanism
operates as follows:
...
exceeding the 100 MB limit or device storage pressure), the SDK fetches fresh
data from the server.

There is no staleness indicator beyond the offline banner. Once connectivity
is restored and listeners have caught up, the UI reflects the current server
...
**Behaviour:** The Firestore SDK persists pending writes to disk when
`persistenceEnabled` is `true`. On the next app launch with connectivity, the
SDK transmits the queued writes. The expense is created on the server and the
`recomputeSimplifiedBalances` function fires normally.
...
### 6.1 Emulator-Based Integration Test
...
### 6.3 Queued Write Persistence Test
