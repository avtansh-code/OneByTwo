...
> Last updated: FR-AC-05 deep-link tab-switch on notification tap (#69, `8dd67f9`) is now **Merged**; the velocity Total is reconciled to **125 SP / 29 PRs** through FR-AC-05 — it closed the last deferred piece of a P0, so **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped, and every P1 is shipped**. The next PR — the **AC-11 "Open Settings" deep-link CTA chore** (FR-AC-04 / AC-11 + the parallel FR-FR-01 contact-permission gap; the highest-ranked unshipped carry-forward candidate, surfaced by PR #55 QA) — opened to add the `app_settings` plugin behind a shared `AppSettingsService` seam and wire the CTA on the SCR-27 notification banner and the SCR-10 contact-permission view (ADR-0019). It lands as the next available GitHub number (≥ #70 now that the highest PR is #69 and the highest issue is #68); there is **no** tracked issue (candidate-list bullet only), so the PR opens without a `Closes #NN`. Reconcile the slot label at PR open.
...
| #70 | 2 | Open (AC-11 "Open Settings" deep-link CTA — `app_settings` plugin + `AppSettingsService` seam + both surfaces + telemetry) |
| **Total** | **125** | **29 PRs so far** |
...
> CI pipeline speed-up) are merged but carry no feature SP. FR-AC-05
> deep-link tab-switch on notification tap (3 SP, P0, the first
> carry-forward candidate) merged as #69 (`8dd67f9`) and is now counted
> in the Total — closing the last deferred piece of a P0, so every P0
> except the Sprint-3 Groups epic and every P1 is shipped. The next PR,
> the AC-11 "Open Settings" deep-link CTA chore (2 SP, the highest-ranked
> unshipped carry-forward candidate), is Open (≥ #70) and its 2 SP will
> be added to the Total on merge.
