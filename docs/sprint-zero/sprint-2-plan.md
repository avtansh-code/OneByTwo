...
> Last updated: the **Issue #47 friendship-expense creator-only rules hardening** (#72, `33f870d`) is now **Merged** — `isValidExpenseUpdate()` now gates friendship-context expense edit and soft-delete on `request.auth.uid == prev.createdBy`. The velocity Total is reconciled to **132 SP / 32 PRs** through #72. With #72 merged, **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped, and every P1 is shipped**, and the single-focus issue-backed carry-forward candidate (#47) is consumed. The next PR — the **Bucket-B close-with-evidence bundle** (the runner-up to #47 named in prompt `31.md`) — is a documentation + issue-tracker-hygiene chore: it verifies the evidence for the Sprint-1 boundary-audit findings that Sprint 2 PRs already resolved, posts per-issue evidence comments, **fully closes #21** and **re-scopes #20 / #23** (kept open), and reconciles the burndown + roadmap. It acts on existing audit issues #20 / #21 / #23 (no new tracked issue) and carries a `Closes #21` line only (the one fully-resolved target). No app code, rules, or new tests. It lands as the next available GitHub number (≥ #73 now that the highest PR is #72 and the highest issue is #68); reconcile the slot label at PR open. Likely **velocity-excluded** (docs/hygiene, like #59) — added to the tables but contributing 0 to the Total unless the architect rules otherwise.
...
| #73 | 0 | Open — Bucket-B close-with-evidence bundle (documentation + issue-tracker hygiene); **velocity-excluded** per the #59 / #61 convention, so its ~3 SP of verification/reconciliation effort counts 0 and it is not in the "PRs so far" tally |
...
> PRs**. The next PR, the **Bucket-B close-with-evidence bundle** (≥ #73), is a
> documentation + issue-tracker-hygiene chore — it verifies the Sprint-1
> boundary-audit findings that Sprint 2 PRs already resolved, fully closes #21,
> and re-scopes #20 / #23. Its ~3 SP of effort is **velocity-excluded** (same
> rule as #59 / #61), so it is added to the tables as Open but contributes **0**
> to the Total. If the architect instead rules it carries SP, that figure is
> stated explicitly here at merge — the Total is never silently inflated.
