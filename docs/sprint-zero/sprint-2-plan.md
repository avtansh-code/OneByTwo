...
> Last updated: the `shared_preferences` cross-launch-persistence chore (#71, `a0de106`) is now **Merged**; the velocity Total is reconciled to **130 SP / 31 PRs** through #71. With #71 merged, **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped, and every P1 is shipped.** The next PR — the **Issue #47 friendship-expense creator-only rules hardening** (the highest-ranked unshipped single-focus, issue-backed carry-forward candidate) — tightens `isValidExpenseUpdate()` so only the expense creator may edit or soft-delete a friendship-context expense (a defence-in-depth re-check of the FR-EX-06 client UI gate; closes the FR-EX-06 §2.9 item 5 gap and the two self-documenting "flip to assertFails" placeholder tests). It **has** a tracked GitHub issue — [#47](https://github.com/avtansh-code/OneByTwo/issues/47) (`sprint-2-chore`) — so the PR opens **with** a `Closes #47` line; it lands as the next available GitHub number (≥ #72 now that the highest PR is #71 and the highest issue is #68). Reconcile the slot label at PR open.
...
| #72 | 2 | Open (Issue #47 friendship-expense creator-only rules hardening — `Closes #47`) |
| **Total** | **130** | **31 PRs so far** |
...
> CI pipeline speed-up) are merged but carry no feature SP. The `shared_preferences`
> cross-launch-persistence chore (3 SP, the highest-ranked unshipped
> carry-forward candidate; the natural pair with the #70 `app_settings`
> chore) merged as #71 (`a0de106`) and is now counted in the Total — so
> with every P0 except the Sprint-3 Groups epic and every P1 shipped, the
> Total stands at **130 SP / 31 PRs**. The next PR, the Issue #47
> friendship-expense creator-only rules hardening (2 SP, the highest-ranked
> unshipped single-focus, issue-backed carry-forward candidate; opens with
> `Closes #47`), is Open (≥ #72) and its 2 SP will be added to the Total on
> merge.
...
| [#47](https://github.com/avtansh-code/OneByTwo/issues/47) | Firestore rules: tighten friendship-expense update/delete to creator-only | — (filed mid-sprint during PR #46; FR-EX-06 §2.9 item 5 / reviewer note R-2) | Open — **NEXT PICK** (prompt `docs/copilot_prompts/sprint_2/31.md` prepared) | `firestore.rules:282-308` `isValidExpenseUpdate()` lacks a `request.auth.uid == prev.createdBy` gate, so **any** friendship member (not only the creator) can edit / soft-delete an expense directly. The two `assertSucceeds` placeholder tests at `functions/test/firestore-rules/expenses-friendship.test.ts:612-647` document the gap (each carries a "flip to assertFails" comment). ~2 SP pure `firestore.rules` + rules-tests chore; opens with `Closes #47`. |
...
| **Standalone rules-hardening chore (~2 SP — NEXT PICK)** | **#47** | Defence-in-depth: restrict friendship-expense edit / soft-delete to the creator, closing the FR-EX-06 §2.9 item 5 gap (the client UI already gates on `createdBy`). Pure `firestore.rules` + rules-tests; no client / trigger / schema change. Prompt `docs/copilot_prompts/sprint_2/31.md` prepared; opens with `Closes #47`. |
