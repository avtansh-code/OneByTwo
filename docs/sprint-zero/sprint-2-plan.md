...
> Last updated: FR-HD-03 current-month spend summary with a category breakdown chart (#67, `1f26548`) is now **Merged**; the velocity Total is reconciled to **122 SP / 28 PRs** through FR-HD-03 — it was the **top-ranked remaining P1** and the **last open Home-dashboard requirement**, and **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped**. The deferred N+1-read denormalised monthly-spend rollup optimisation is filed as FUTURE issue #68 (do not build it). FR-AC-05 (deep-link tab-switch, SRS section 4.7 line 240, **P0**) opened as the next PR — the first carry-forward candidate, unblocked by the `shellNavigationControllerProvider` seam (#63) and closing that PR's "controller seam only" deferral; it lands as the next available GitHub number (≥ #69 now that FUTURE issue #68 is filed; reconcile the slot label at PR open).
...
| #69 | 3 | Open (FR-AC-05 deep-link tab-switch on notification tap) |
| **Total** | **122** | **28 PRs so far** |
...
> CI pipeline speed-up) are merged but carry no feature SP. FR-HD-03
> current-month spend category breakdown chart (5 SP, the top-ranked
> remaining P1 and last open Home-dashboard requirement) merged as #67
> (`1f26548`) and is now counted in the Total. The next PR, FR-AC-05
> deep-link tab-switch on notification tap (3 SP, P0, the first
> carry-forward candidate), is Open (#69) and its 3 SP will be added to
> the Total on merge.
