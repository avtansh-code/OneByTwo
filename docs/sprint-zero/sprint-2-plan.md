...
> Last updated: FR-PR-04 My Friends / My Groups from Profile (#63, `209afea`) is now **Merged**; the velocity Total is reconciled to **104 SP / 25 PRs** through FR-PR-04 — **every P0 functional requirement is now shipped**. FR-PR-02 (update phone number via OTP re-verification, SRS section 4.2 line 175, **P1**) opened as the next feature PR — the top-ranked remaining P1 on the carry-forward candidate list (above FR-AU-09 account-deletion and the chore bundle) and lands as the next available GitHub number (≥ #64).
...
> feature PR, FR-PR-02 update phone number via OTP re-verification (5 SP,
> the top-ranked remaining P1 now that every P0 functional requirement is
> shipped; lands as the next available GitHub number ≥ #64), is Open and
> will be added on merge.
