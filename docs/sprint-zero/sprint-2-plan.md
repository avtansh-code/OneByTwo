...
> Last updated: the AC-11 "Open Settings" deep-link CTA chore (#70, `dda2f97`) is now **Merged**; the velocity Total is reconciled to **127 SP / 30 PRs** through #70. With #70 merged, **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped, and every P1 is shipped.** The next PR — the **`shared_preferences` cross-launch-persistence chore** (the highest-ranked unshipped carry-forward candidate; the natural pair with the #70 `app_settings` chore, deliberately NOT bundled into it) — opened to adopt `shared_preferences` behind a thin `lib/core/services/KeyValueStore` seam and persist the two documented in-memory deferrals (FR-AC-04 `wasPermanentlyDenied` and the FR-SE-09 reminder cooldown) across launches (ADR-0020). There is **no** tracked GitHub issue (candidate-list bullet + two in-code TODOs only), so the PR opens without a `Closes #NN`; it lands as the next available GitHub number (≥ #71 now that the highest PR is #70 and the highest issue is #68). Reconcile the slot label at PR open.
...
| #71 | 3 | Open (`shared_preferences` cross-launch persistence — `KeyValueStore` seam + `wasPermanentlyDenied` + FR-SE-09 cooldown + expiry guard) |
| **Total** | **127** | **30 PRs so far** |
...
> CI pipeline speed-up) are merged but carry no feature SP. The AC-11
> "Open Settings" deep-link CTA chore (2 SP, the highest-ranked unshipped
> carry-forward candidate) merged as #70 (`dda2f97`) and is now counted
> in the Total — so with every P0 except the Sprint-3 Groups epic and
> every P1 shipped, the Total stands at **127 SP / 30 PRs**. The next PR,
> the `shared_preferences` cross-launch-persistence chore (3 SP, the
> highest-ranked unshipped carry-forward candidate; the natural pair with
> the #70 `app_settings` chore), is Open (≥ #71) and its 3 SP will be
> added to the Total on merge.
