...
> Last updated: FR-HD-01/02 Home dashboard (#62, `57c272e`) is now **Merged**; the velocity Total is reconciled to **101 SP / 24 PRs** through FR-HD-01/02. FR-PR-04 (My Friends / My Groups from Profile, SCR-26, **P0**) opened as the next feature PR — it was missing from the `next-three-prs.md` candidate list (an omission corrected here: it is the last open P0 functional requirement) and lands as the next available GitHub number (≥ #63).
...
> CI pipeline speed-up) are merged but carry no feature SP. The next
> feature PR, FR-PR-04 My Friends / My Groups from Profile (3 SP, the
> last open P0 functional requirement; lands as the next available
> GitHub number ≥ #63), is Open and will be added on merge.
