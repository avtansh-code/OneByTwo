...
> Last updated: the **Issue #47 friendship-expense creator-only rules hardening** (#72, `33f870d`) is now **Merged** — `isValidExpenseUpdate()` now gates friendship-context expense edit and soft-delete on `request.auth.uid == prev.createdBy`. The velocity Total is reconciled to **132 SP / 32 PRs** through #72. With #72 merged, **every P0 functional requirement except the deferred Sprint-3 Groups epic (FR-GR-01..07) is shipped, and every P1 is shipped**, and the single-focus issue-backed carry-forward candidate (#47) is consumed. The **Bucket-B close-with-evidence bundle** has since **Merged** as #73 (`f9a4c54`) — it verified the Sprint-1 boundary-audit findings that Sprint 2 PRs already resolved, posted per-issue evidence comments, **fully closed #21**, **re-scoped #20 / #23** (kept open), and reconciled the burndown + roadmap; it was **velocity-excluded** (docs/hygiene, like #59), so the Total is unchanged at **132 SP / 32 PRs**. The next PR — the first of the three consolidated Sprint-2 close-out PRs — is the **`chore(auth)` client-housekeeping bundle** (#74): it renames `authStateNotifierProvider` → `authStateProvider` (M1), relocates the three shared providers (`firebaseFirestoreProvider`, `firebaseStorageProvider`, `phoneAuthRepositoryProvider`) to `lib/core/providers/` (M4), emits the four secondary auth-funnel telemetry events and corrects the `is_new_user` typing note (T3/T4/T5), and aligns the splash / phone-entry / OTP screen specs (S1/S3/S4/S5); it **closes #15 / #16 / #17 / #18** and **carries 5 SP** (feature-adjacent client work). It lands as the next available GitHub number (≥ #74 now that the highest PR is #73 and the highest issue is #68); reconcile the slot label at PR open.
...
| #74 | 5 | Open — `chore(auth)` client-housekeeping bundle (closes #15 / #16 / #17 / #18); feature-adjacent client work that **carries 5 SP**, counted in the Total on merge per the rolling-reconciliation convention |
| **Total** | **132** | **32 PRs so far** |
...
> (`33f870d`) and is now counted in the Total — so with every P0 except the
> Sprint-3 Groups epic and every P1 shipped, the Total stands at **132 SP / 32
> PRs**. The **Bucket-B close-with-evidence bundle** merged as #73 (`f9a4c54`);
> as a documentation + issue-tracker-hygiene chore it is **velocity-excluded**
> (same rule as #59 / #61), so it contributes **0** and the Total is unchanged.
> The next PR — the **`chore(auth)` client-housekeeping bundle** (#74, closes
> #15 / #16 / #17 / #18) — is feature-adjacent client work that **carries 5 SP**;
> per the rolling-reconciliation convention it is added as Open now and its SP
> is counted in the Total when the following PR reconciles it to Merged. The
> Total is never silently inflated.
...
| **#76** | `test` | **#20, #23** | SC1 concurrent-submit-guard (phone entry), SC2 OTP-auto-retrieval-timeout, SC3 `MAX_SAFE_INTEGER` overflow, **SC4 large-group (100+) algorithm scalability** tests — **fully closes #20** + RT2 CI step-duration logging + INV2 share-sheet boundary verification + a Flutter integration-test harness realising the skipped `test/integration/**/*_flow_test.dart` stubs (PY3) — **aims to fully close #23**. | 5 |
