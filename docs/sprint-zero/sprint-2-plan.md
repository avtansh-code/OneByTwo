...
> Last updated: FR-PR-02 update phone number via OTP re-verification (#64, `2e68713`) is now **Merged**; the velocity Total is reconciled to **109 SP / 26 PRs** through FR-PR-02 — **every P0 functional requirement is shipped and FR-PR-02 was the top-ranked remaining P1**. FR-AU-09 (permanently delete your account, SRS section 4.1 line 168, **P1**) opened as the next feature PR — the next top-ranked P1 on the carry-forward candidate list (above the Bucket-B chore bundle, Issue #47 rules-hardening, and the `shared_preferences` / `app_settings` chores) and lands as the next available GitHub number (≥ #65).
...
| #65 | 8 | Open (FR-AU-09 delete account + cascade cloud function) |
| **Total** | **109** | **26 PRs so far** |
...
> CI pipeline speed-up) are merged but carry no feature SP. FR-PR-02
> update phone number via OTP re-verification (5 SP) merged as #64
> (`2e68713`) and is now counted in the Total. The next feature PR,
> FR-AU-09 permanently delete your account (8 SP, the next top-ranked P1;
> lands as the next available GitHub number ≥ #65), is Open and its 8 SP
> will be added to the Total on merge.
