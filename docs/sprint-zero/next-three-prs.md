...
> Last updated: the **Issue #47 friendship-expense creator-only rules hardening** (#72, `33f870d`) merged — `isValidExpenseUpdate()` now gates friendship-context expense edit / soft-delete on `request.auth.uid == prev.createdBy`. With it, **every P0 functional requirement except the deferred Groups epic (FR-GR-01..07, now Sprint 4) is shipped, every P1 is shipped**, and the single-focus issue-backed carry-forward candidate (#47) is consumed. The **Bucket-B close-with-evidence bundle** has since merged as #73 (`f9a4c54`) — it verified the Sprint-1 boundary-audit findings that Sprint 2 PRs already resolved, posted per-issue evidence comments, **fully closed #21**, **re-scoped #20 / #23** (kept open), and reconciled the burndown + roadmap; **velocity-excluded** (docs/hygiene, like #59). The **`chore(auth)` client-housekeeping bundle** (#74, `ce6d594`) has since merged — the first of the three consolidated Sprint-2 close-out PRs (renamed `authStateNotifierProvider` → `authStateProvider` (M1), relocated the three shared providers to `lib/core/providers/` (M4), emitted the four secondary auth-funnel telemetry events and corrected the `is_new_user` typing note (T3/T4/T5), and aligned the splash / phone-entry / OTP screen specs (S1/S3/S4/S5)); it **closed #15 / #16 / #17 / #18** and carried **5 SP**, advancing the velocity Total to **137 SP / 33 PRs**. The **`docs` close-out bundle** (#75, `dbc209d`) has since **Merged** — the second of the three: it added the #19 (CV2) coverage fields, completed the #24 (CN3 / CN4) Jest-config table and Cloud-Functions PR checklist, and created the #28 (SR3) Friends HTML mockup; it **closed #19 / #24 / #28** and was **velocity-excluded** (0 SP, pure docs/design). The next PR — the **third and final** of the three — is the **`test` close-out bundle** (#76): the SC1–SC4 coverage tests, RT2 CI step-duration logging, and the INV2 share-sheet verification; it **closes #20** and **re-scopes #23** (RT2 + INV2 closed with evidence; the PY3 Flutter emulator-harness remainder kept open), and **carries 5 SP** (counted on merge: 137 / 33 → 142 / 34). It lands as the next available number (≥ #76 now that the highest PR is #75 and the highest issue is #68), reconciled at PR open. **CLOSED:** #76 merged (`3f3cd16`, **142 / 34**) and #77 merged (`b9b1e63`, path-filter pipeline + milestone-tracking convention + deepened review skill; velocity-excluded), so **Sprint 2 is fully closed.** The **Sprint 2 → Sprint 3 boundary audit** then ran: cleanup **PR #91** (`chore: Sprint 2 boundary cleanup and audit findings`) fixes the 33 Bucket-A findings and files 44 Bucket-B issues (#78–#90); audit + cleanup, velocity-excluded. **RENUMBER: a Design Conversion sprint was inserted as the new Sprint 3 (Haldi visual-system migration; ADR-0024); the Groups epic shifts to Sprint 4. MERGED (#126, `d9205d8`) — Sprint 3 (Design Conversion) PR #1: DC-01 design-token foundation** (issue #113, branch `feat/design-token-foundation`) — `lib/app/theme.dart` → Haldi marigold `ColorScheme` with **ink** `onPrimary` + the `OBTColors` extension + the `AppTheme` radius / soft-warm-shadow / motion tokens + Bricolage Grotesque / Hanken Grotesk + the `OBTText.amount` / `.amountHero` helpers, per `docs/sprint-zero/sprint-3-plan.md`; it changes no application behaviour, data model, security rules, the simplified-debts algorithm, or Cloud Functions, and reskins **no** screen or `OBT*` widget (those are DC-02 onward). DC-01..DC-13 are filed under the Sprint 3 milestone (#113–#125). **MERGED (#127, `aca8582`) — Sprint 3 (Design Conversion) PR #2: DC-02 (#114)** — reskinned the six `OBT*` shared widgets to the Haldi tokens (branch `feat/obt-widget-reskins`); token/type reskin only, no structural change, no screen converted, `IndianPhoneInputFormatter` conforms; the FAB resolves **ink** on marigold (`onPrimary`, never white), `OBTAmountInput` keeps its integer-paise `onChanged(int paise)` contract and the `formatInrFromPaise()` boundary (Invariant 1), and `OBTSettleUpCard` keeps the single `simplifiedBalances` suggested-payment read (Invariant 2). **MERGED (#129, `b0ce9a6`) — Sprint 3 (Design Conversion) PR #3: DC-03 (#115)** — build the **eleven** new non-Groups Haldi shared components (skeleton-loader set, `OBTBalancePill`, category chip/tile, 3-step add-expense sheet shell, segmented split-method control, settle-up bottom sheet, empty-state scaffold, offline/pending-sync banner, monthly-spend donut + legend, OTP six-box input, brand kit) as reusable, fully-stated shared widgets **in isolation** — four states each (populated, empty, shimmer-loading, error) + queued light/dark goldens; branch `feat/haldi-shared-components`. **No screen is converted** (DC-04 onward), the inert "Pay via UPI" slot stays "Coming soon", and the two **Groups-specific** components — #10 group segmented tab bar and #12 stacked-avatar cluster — are **deferred to Sprint 4**. **MERGED (#131, `0391bf2`) — Sprint 3 (Design Conversion) PR #5: DC-04 (#116)** — convert the **Auth & onboarding** flow to Haldi (branch `feat/haldi-auth-conversion`): **rebuild** splash (Haldi 1) on the brand kit (**ink** on marigold); **build net-new onboarding** (Haldi 2) — three slides (Track / Split / Settle) + pagination dots + **Skip** + an ink-on-marigold "Get started" + **Terms / Privacy** links, gated behind a first-launch "seen" flag via `shared_preferences`; and **reskin** phone-entry (3), OTP (4, adopting `OBTOtpInput`) and profile-setup (5, camera badge → **onPrimary ink, not white**), `+91` stays locked. The standalone `otp_input.dart` is **retained** (still used by the Profile change-phone flow / DC-10), and the orphaned `authenticated_screen.dart` is **deleted** (no inbound references). All **four invariants hold unchanged** — the Auth surface touches no money / balance / share / Firebase-project path. **MERGED (#132, `1c3e194`) — Sprint 3 (Design Conversion) PR #6: DC-05 (#117)** — convert the **Home dashboard** hero to Haldi (branch `feat/haldi-home-conversion`): the net-balance header becomes a tonal `surfaceContainerHighest` hero with the marigold `OBTColors.heroShadow` and a Bricolage `OBTText.amountHero` amount carrying the **balance trio (colour + icon + label)**; the hand-rolled skeleton → the shimmer `OBTSkeleton` set; the empty `Icon` → `OBTEmptyState`; the top-balance tile adopts the shared `OBTBalancePill` (with icon) + a marigold-link Settle Up (reflowing to a stacked layout at narrow widths / large dynamic type so amounts never truncate); the spending donut + legend re-point to the Haldi 8-hue (`OBTColors.category`). The Groups-tab stub `groups_list_placeholder.dart` is **deleted, not converted** — the shell tab-2 swaps to an inline Haldi `OBTEmptyState` (`groups_coming_soon_tab.dart`) so the 5-tab + FAB structure conforms. **Invariants 1 + 2 are LIVE and hold** — every on-screen amount routes through `formatInrFromPaise()` and `simplifiedBalances` stays a read-only projection; Invariants 3 + 4 untouched. `Closes #117`. **MERGED (#133, `006a0fb`) — Sprint 3 (Design Conversion) PR #7: DC-06 (#118)** — convert the **Friends** flow to Haldi (branch `feat/haldi-friends-conversion`): **reskin** Friends list (Haldi 9 ★) — the additive owed/owe summary band + `OBTBalancePill` rows (colour + icon + label) + the shimmer `OBTSkeleton` set + `OBTEmptyState`; **reskin + rebuild** Add-friend (Haldi 10) — Haldi tokens across the segmented Contacts/Manual entry, and the `match_and_invite_screen` **rebuilt** with Haldi guard states (RateLimited / SelfAddBlocked / DuplicateFriendship) + a shimmer looking-up state + a match-found confirm-add card + a no-match `OBTEmptyState` invite that hands off to the **OS system share sheet only** (AC-3 / Invariant 3); **reskin** Friend detail (Haldi 11) — the header large balance pill → `OBTBalancePill`, the `OBTSettleUpCard`, the Haldi category-hue timeline, plus a "View full history" affordance; and **build net-new** Friend history (Haldi 12; FR-FR-04) — a read-only, reverse-chronological, month-grouped (IST) log of expenses **and** settlements with **signed amounts** via `formatInrFromPaise()`, in four states. The shared-feature `balance_pill.dart` is **deleted** and every call site re-points to `OBTBalancePill`. **Invariants 1 + 2 + 3 are LIVE and hold** — every amount routes through `formatInrFromPaise()`, `simplifiedBalances` stays a read-only projection, and the invite path targets only the system share sheet; Invariant 4 untouched. `Closes #118`. **MERGED (#134, `262107d`) — Sprint 3 (Design Conversion) PR #8: DC-07 (#119)** — convert the **Expenses** flow to Haldi (branch `feat/haldi-expenses-conversion`): the Add-expense 3-step sheet (Haldi 21 ★) gains the 28-radius top + grabber + visual stepper via `OBTStepperSheet`, **step 2 is rebuilt** to `OBTSegmentedSplitControl` (Equally / Unequal / % / Shares / Exact) with live "adds up" green / over-under red validation and Next disabled when the split does not sum to the total (AC-2 / Invariant 1), and **step 3** adds an inert note slot + the inert "Make recurring" slot ("Coming soon"); the Expense detail (Haldi 22) renders the per-person **simplified** split with the balance trio **read from the projection** (Invariant 2), a shimmer skeleton, and the IST date; and the add-expense context picker (Haldi 8) shows Groups "Coming soon" with an additive search field, friend selection still opening the add sheet. **Invariants 1 + 2 are LIVE and hold** — every amount via `formatInrFromPaise()` and `simplifiedBalances` stays a read-only projection; Invariants 3 + 4 untouched. `Closes #119`. **MERGED (#135, `b55e9fb`) — Sprint 3 (Design Conversion) PR #9: DC-08 (#120)** — convert the **Settlements** flow to Haldi (branch `feat/haldi-settlements-conversion`): the settle-up bottom sheet (Haldi 23 ★) is **rebuilt** onto the shared `OBTSettleUpSheet` — the single `simplifiedBalances` suggested-payment header (never a who-owes-who debt graph), the editable `OBTAmountInput` (the integer-paise contract untouched), the inert "Pay via UPI" "Coming soon" slot, and the in-sheet **success moment** (check + a single haptic) that replaces the old auto-dismiss + "Settlement recorded." snackbar (AC-1); the bespoke `settle_up_header.dart` is **deleted** (superseded by the component's own header); and the settlement history (Haldi 24) is reskinned — marigold tokens + Bricolage tabular, the spinner → the shimmer `OBTSkeleton` set, each row gaining a **sent/received direction icon + sign** derived from `fromUserId`/`toUserId` (the balance trio), the empty `Icon` → `OBTEmptyState`, and IST dates via the core `formatIstLongDate()`. The **reminder-compose** and the **friend/group filter** stay PM-reconcile flags (§C item 6), not built here. **Invariants 1 + 2 are LIVE and hold** — every amount via `formatInrFromPaise()`, the suggestion a single read-only projection read with recompute server-side; Invariants 3 + 4 untouched. `Closes #120`. **MERGED (#136, `1ce84cc`) — Sprint 3 (Design Conversion) PR #10: DC-09 (#121)** — convert the **Activity & Notifications** surfaces to Haldi (branch `feat/haldi-activity-notifications-conversion`): the **Activity feed (Haldi 25 ★)** reskins to marigold tokens + Bricolage/Hanken type, the static grey skeleton becomes the shared shimmer `OBTSkeleton` set (reduced-motion freeze), the hand-rolled empty `Icon` becomes `OBTEmptyState`, the error greys re-point to `OBTColors.metaText`, rows stay on the frozen `OBTActivityRow` and still deep-link unchanged (FR-AC-02), in light **and** dark; the **Notifications surfaces (Haldi 26)** re-point the two hard-coded hexes in the in-app banner (`#2A9D8F` success → `OBTColors.balancePositive`, `#F4A261` secondary → `OBTColors.warning`), move the banner card radius to `radiusCard` (20) with the warm `OBTColors.rowShadow` (light) / outline (dark) and Hanken type, and reskin the "Stay in the loop" pre-permission dialog (radius → card 20, CTA → `onPrimary` ink + `radiusButton` 16, literal `fontSize` → a `textTheme` slot); the `notifications_lifecycle_host` **conforms** (no visual surface of its own). **No hard-coded Haldi hex remains** (AC-3, asserted by a no-`Color(0x…)` grep over the converted banner + dialog). **Invariants 1 + 2 are present and hold** — the only on-screen amount is the feed-row trailing amount rendered inside the frozen `OBTActivityRow` via `formatInrFromPaise()`, and no surface reads or writes `simplifiedBalances`; Invariants 3 + 4 untouched. `Closes #121`. **MERGED (#137, `d58581b`) — Sprint 3 (Design Conversion) PR #11: DC-10 (#122)** — convert the **Profile & Settings** cluster to Haldi (branch `feat/haldi-profile-conversion`): **reskin** Profile view (Haldi 27) — marigold tokens + Bricolage/Hanken, avatar fallback on `primaryContainer` ink, the inline sign-out `AlertDialog` → `OBTConfirmationDialog`, delete → error token, and the hand-rolled `_ShimmerEffect` (with its hard-coded greys) → the shared `OBTSkeleton` set; **reskin** Edit-profile (27) — camera badge → **onPrimary ink (not white)**, field radius → `radiusChipInput`, Save → `radiusButton`, the photo sheet → `radiusSheet`, `+91` locked; **reskin** Change-phone (Phase3g) — marigold + Bricolage/Hanken across the five security-gated states, masked numbers, `+91` locked, and the ADR-0015 "sync pending → Try again" recovery on the **warning** token (not danger), the two-OTP re-auth state machine behaviour-frozen (AC-2); **reskin** Notification preferences (28) — marigold toggles, the OS-permission banner → a warm-tonal card, an inert "Language" "Coming soon" slot, page spinner → `OBTSkeletonList`; **reskin** Delete-account (30) — destructive → error token, the warning callout → `radiusCard`, fields → `radiusChipInput`, buttons → `radiusButton`, the re-auth `OtpInput` and server cascade unchanged; **reskin** Contact-support (29) — dialog radius → `radiusCard`, selectable email → the `OBTColors.link` token; and **reskin** the photo-source chooser (`photo_picker_sheet.dart`, 5/27) — Hanken header, destructive "Remove" → error token. The superseded `profile_placeholder_screen.dart` is **deleted, not converted** (AC-3). Every direct `AppTheme.radiusLarge`/`radiusXL`/`radiusMedium` in the cluster is migrated to the named scale and **no hard-coded Haldi hex remains** (asserted by a no-`Color(0x…)` grep over the converted surfaces). All **four invariants hold unchanged** — the Profile surface renders no on-screen amount and reads/writes no `simplifiedBalances`, keeps Contact Support on the FR-PR-05 `mailto:` flow (no new share channel), and changes no Firebase-project wiring; `+91`-only Phone Auth and the account-deletion contract are untouched. `Closes #122`. **MERGED (#138, `c6bb785`) — Sprint 3 (Design Conversion) PR #12: DC-11 (#123)** — verify dark-mode parity across the **six hero screens** (branch `feat/haldi-dark-mode-parity`): the first PR of **Epic D (Dark-Mode Parity)**. Converts no new screen — every hero was token-converted in DC-04..DC-09; DC-11 **verifies** each hero (Splash 1, Home 6, Friends 9/10/11/12, Add-expense 21/22, Settle-up 23/24, Activity 25) renders the Haldi **dark** tokens (background `#1A1510`, surface `#241D16`, primary `#EAA24A`, `onPrimary` ink `#1A1510`), **fixes** the dark-mode gaps (the `rowShadow`-collapses-to-a-1px-`outline`-border swap for row/tile separation per `friend_list_tile.dart`; any literal light foreground on a warm `error`/`secondary`/`primary` fill re-pointed to `colorScheme.on*` ink — the warm-fill-takes-ink rule, with the white-on-dark-danger negative case asserted to FAIL the contrast gate), and **locks** it with dark-mode widget tests (no overflow at 390/320 + 2.0x dynamic type, outline-not-shadow separation, reduced-motion) + dark golden scaffolds (queued in `dc04..dc09`, group-skipped pending DC-13). All **four invariants hold unchanged** — dark-mode work is presentation-only (no money path, no `simplifiedBalances` write, the system share sheet untouched, single Firebase project). `Closes #123`. **MERGED (#139, `43e2d3d`) — Sprint 3 (Design Conversion) PR #13: DC-12 (#124)** — re-verify accessibility against the new palette (branch `feat/haldi-accessibility-reverification`): the only PR of **Epic E (Accessibility Re-Verification)**. Converts no screen and adds no behaviour — it **re-verifies** the shipped Haldi palette + type + motion meet **WCAG 2.1 AA** across the conversion and **wires the contrast + dynamic-type gates into CI as a named, blocking `a11y-checks` job** ("Accessibility Gate (WCAG AA)", `flutter test --tags "a11y-contrast || a11y-dynamic-type"` — the boolean-OR selector; the repeated `--tags a --tags b` form is *last-wins* and would silently run only the last family). **AC-1:** the verified pairings hold against the *resolved* tokens, with the published handoff figures reconciled against the Flutter-measured ratios (`6.25 / 14.66 / 5.04 / 5.36 / 15.31`) in `docs/audits/design-conversion/04-qa-test-strategy.md` §B.1 (no token change — the gate tracks the resolved-token reality; any token tuning routes to #128). **AC-2:** every converted hero renders at 2.0x at 390 **and** 320, light + dark — the expense-detail per-person split row (`_SplitBalanceRow`) gains a `LayoutBuilder` stacked-reflow + `Wrap` so the rupee figure never truncates (the one real AA gap the re-verification surfaced; the `OBTSettleUpSheet` / `OBTActivityRow` 320 overflows stay the documented #128 exceptions). **AC-3:** every balance signal survives greyscale / colour-blind simulation via **icon + label** (never colour alone), with the colour-only negative case asserted to FAIL, and reduced motion collapses the page transition + freezes skeletons. Goldens remain DC-13's (no PNG authored here); `lib/app/theme.dart` / `core/theme/*` / the DC-02/03 components are untouched. All **four invariants hold unchanged** — accessibility verification touches no money / `simplifiedBalances` / share / Firebase-project path. `Closes #124`. **IN FLIGHT — Sprint 3 (Design Conversion) PR #14: DC-13 (#125)** — author the golden baselines and wire the visual-regression harness (branch `feat/haldi-golden-harness`): the only PR of **Epic F (Visual-Regression / Golden-Test Harness)** and the **last** Design-Conversion PR. Converts no screen and adds no behaviour — it bundles the Bricolage Grotesque + Hanken Grotesk OFL fonts for deterministic offline rendering, un-skips the nine golden groups (the foundation showcase, the six `OBT*` reskins, the eleven DC-03 components across their four states, and the DC-04..DC-10 hero / flow matrices in light + dark), authors the baselines on `ubuntu-latest`, and wires the compare-only `golden-a11y-checks` CI job; it changes no application behaviour, token, data path, or security rule, and all four invariants hold unchanged. **On merge, Epic F and Sprint 3 close**; the remaining open Sprint-3 items #128 (foundation reconciliation) and #110 (production deploy-drift) are tracked separately and are not blockers for the Epic-F close. `Closes #125`. **The Groups epic is now Sprint 4:** FR-GR-01 (Create group, issue #92) is the first Sprint-4 PR and is unblocked (DoR-compliant story `docs/sprint-zero/stories/FR-GR-01-create-group.md`; the `groups/{groupId}` schema, create rule, screen spec SCR-14, wireframe, mockup, and `group_created` telemetry all exist; see `docs/sprint-zero/sprint-4-kickoff-readiness.md`). **FR-GR-02 (#93)** and **FR-GR-03 (#94)** follow, building on the invite-token model (ADR-0023) added in #91. **IN FLIGHT — Sprint 3 → Sprint 4 boundary sweep: cleanup PR #144** (`fix: Sprint 3 design-conversion cleanup and audit findings`, branch `fix/sprint-3-design-conversion-cleanup`) — the design-conversion audit + remediation (`docs/audits/sprint-3/`). Bucket A fixed in #144: **9 rupee-glyph (₹-in-Hanken) tofu renders** swapped to Bricolage `OBTText.amount*` or the new `OBTText.rupeeAware` fallback (incl. two decoupled domain→presentation renders the golden image review caught — the split-validation banner and the amount-input error), **7 single-line-fit amount figures** wrapped in `FittedBox(scaleDown)`, the `expectGoldenState` golden guards on dc07/08/09, **ADR-0026** + coding-standards back-ports, and supersession banners on the 21 Haldi-superseded visual docs; goldens regenerated on ubuntu and image-reviewed. Bucket B filed: **#145** (require Golden & A11y + Accessibility Gate checks — Sprint 4, owner action), **#146** (verify nightly deploy green — Sprint 4), **#147** (dependency refresh — Post-v1.0). **Velocity-excluded** (audit + cleanup, like the Sprint-1/2 retros). On merge, **Sprint 3 is fully closed and the Sprint-4 Groups epic flips to next-up**: the next PR is **FR-GR-01 (Create group, #92)**, which builds the group Haldi components first (group list tile, member row, invite-link card, group settle-up, tab bar — #79) so Groups is Haldi-native, contingent on the #145 required-checks owner action.
