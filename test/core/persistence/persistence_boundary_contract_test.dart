// Boundary-contract tests for the shared_preferences cross-launch
// persistence chore's new core files.
//
// Mirrors the per-feature boundary-contract pattern (e.g.
// test/core/services/permission_settings_boundary_contract_test.dart)
// but scans the two NEW shared core files this chore adds:
//   - lib/core/services/key_value_store.dart
...
//     `'friendship_id_hash'`. On-device key names are exempt from the
//     PII rule (it governs Analytics/Crashlytics), but they must still
//     never embed those Analytics-parameter literals.
...
void main() {
  group('persistence boundary contract — files exist', () {
  ...
  group('persistence boundary contract — no double / no /100 '
      '(invariant 1)', () {
  ...
  group('persistence boundary contract — no simplifiedBalances '
      '(invariant 2)', () {
  ...
  group('persistence boundary contract — PII-leak guard', () {
